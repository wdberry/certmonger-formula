...
We will see a quick setup to configure the ``certmonger`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``certmonger`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``certmonger`` formulas under ``/srv/salt/certmonger/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/certmonger/parameters/dns:domain/
    ln -s dns:domain /srv/salt/certmonger/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/certmonger/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/certmonger/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``certmonger`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``certmonger`` formulas under ``/srv/salt/certmonger/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/certmonger/parameters/roles
...
We create a configuration for the role ``certmonger/server`` in ``/srv/salt/certmonger/parameters/roles/certmonger/server.yaml``:
...
We create another configuration for the role ``certmonger/client`` in ``/srv/salt/certmonger/parameters/roles/certmonger/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles certmonger/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles certmonger/client
...
.. code-block:: console
    ...
    mv /srv/salt/certmonger/parameters/dns:domain/  '/srv/salt/certmonger/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-certmonger-as-json:
      test.nop:
        - name: {{ certmonger | json }}
...
Here is an example based on `template-formula/certmonger/config/file.sls`_:
...
This ``sls`` file expose a ``certmonger`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ certmonger['winner'] }}
...
.. _template-formula/certmonger/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/certmonger/config/file.sls
